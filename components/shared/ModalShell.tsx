...
export function ModalShell({ title, subtitle, onClose, children, wide = false }: { title: string; subtitle?: string; onClose: () => void; children: React.ReactNode; wide?: boolean }) {
  const category = title.toLowerCase().includes("mật khẩu") ? "TÀI KHOẢN" : title.toLowerCase().includes("buổi") ? "BUỔI DẠY" : title.toLowerCase().includes("học sinh") ? "HỌC SINH" : title.toLowerCase().includes("môn") ? "MÔN HỌC" : "XÁC NHẬN";
